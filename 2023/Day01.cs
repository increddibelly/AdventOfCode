...
7pqrstsixteen";
	...
	{
		var result = source.Split(Environment.NewLine).Select(x => GetNumber(x)).Sum();
	...
	private static IEnumerable<int> OnlySmartNumbers(string input, bool first = true) {
		...
		var lastNr = numbers.Select(nr => first ? input.IndexOf(nr) : input.LastIndexOf(nr)).Max();
		while (lastNr >= 0) 
		{
			Console.Write($"{input} => ");
			var oldNumber = numbers.Single(nr => input[lastNr..].StartsWith(nr));
			input = input
				.Replace(oldNumber.ToString(), map[oldNumber].ToString());
			Console.WriteLine(input);
			
			lastNr = numbers.Select(nr => first? input.IndexOf(nr) : input.LastIndexOf(nr)).Max();
		}
		Console.WriteLine();
